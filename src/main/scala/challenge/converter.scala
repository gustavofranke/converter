...
/**
  * Takes a String representing match time in one format,
  * and convert it to a String representing match time in another format.
  *
 * The input match time is in the format {{{[period] minutes:seconds.milliseconds}}}
  *
 * The output should be in the format {{{normalTimeMinutes:normalTimeSeconds - period}}}
  */
object Converter {
  def parseMatchTime(s: String): Either[String, MatchTime] =
    s match {
  ...
  def renderMatchTime(emt: Either[String, MatchTime]): String =
    emt.fold(
      _ => Invalid.invalid,
      mt => mt.show
    )
...
object Period {
  def apply(s: String): Either[String, Period] =
    s match {
...
object Minutes {
  val creationImpossible                        = "Minute values are >= 0 and <= 120"
  def apply(s: String): Either[String, Minutes] =
    for {
      i <- parseInt(s)
      r <- Either.cond(i >= 0 && i <= 120, new Minutes(i) {}, creationImpossible)
    } yield r
}
...
object Seconds {
  val creationImpossible = "Second values are >= 0 and <= 60"

  def apply(s: String): Either[String, Seconds] =
    for {
      i <- parseInt(s)
      r <- Either.cond(i >= 0 && i <= 60, new Seconds(i) {}, creationImpossible)
    } yield r

  def apply(i: Int): Either[String, Seconds] =
    Either.cond(i >= 0 && i <= 60, new Seconds(i) {}, creationImpossible)
}
...
object MilliSeconds {
  val creationImpossible                             = "MilliSecond values are >= 0 and <= 999"
  def apply(s: String): Either[String, MilliSeconds] =
    for {
      i <- parseInt(s)
      r <- Either.cond(i >= 0 && i <= 999, new MilliSeconds(i) {}, creationImpossible)
    } yield r
}
...
/**
  ...
  */
sealed abstract case class MatchTime(period: Period, minutes: Minutes, seconds: Seconds, milliseconds: MilliSeconds)

object MatchTime {
  def apply(period: String, minutes: String, seconds: String, milliseconds: String): Either[String, MatchTime] =
    for {
      per <- Period(period)
      min <- Minutes(minutes)
      sec <- Seconds(seconds)
  ...
  implicit val showMatchTime: Show[MatchTime] = Show.show[MatchTime] { mt =>
    ...
    Seconds(if (mt.milliseconds.s >= 500) mt.seconds.s + 1 else mt.seconds.s)
      .fold(
        _ => Invalid.invalid,
        roundedMillisInSecs =>
          mt.minutes.s match { // scalastyle:off magic.number
